...
const MoMoPayment = () => {
  ...
  const handleMoMoPay = async () => {
    ...
    } catch (err) { addToast("Payment failed.", "error"); setProcessing(false); }
  };
    ...
    <div className="min-h-screen bg-pink-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl overflow-hidden border border-pink-100">
        ...
        <div className="p-10 text-center">
          <div className="bg-white p-4 border-2 border-pink-100 rounded-3xl mb-8 inline-block shadow-inner"><QrCode size={160} className="text-gray-800" /></div>
          <div className="space-y-4">
            <Button onClick={handleMoMoPay} fullWidth isLoading={processing} className="bg-[#A50064] hover:bg-[#80004d] py-4 text-lg rounded-2xl">Pay with MoMo</Button>
            <button onClick={handleCancel} disabled={processing} className="w-full flex items-center justify-center py-3 text-gray-400 hover:text-pink-600 font-bold"><XCircle className="mr-2" size={20}/> Stop Transaction</button>
          </div>
        </div>
      </div>
    </div>
